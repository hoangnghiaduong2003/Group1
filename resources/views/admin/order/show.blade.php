...
<ol class="breadcrumb">
  ...
  <li><a href="{{ route('admin.order.index') }}"><i class="fa fa-list-alt" aria-hidden="true"></i> Order Management</a></li>
...
@section('content')
  ...
  <section class="invoice" style="margin: 0;">
        ...
        <div class="row">
          <div class="col-md-3 col-sm-3 col-xs-3">
            ...
            <address>
              ...
              Email: hoangnghiaduong0@gmail.com<br>
              Address: Ha Noi
            </address>
          </div>
          <div class="col-md-3 col-sm-3 col-xs-3">
          ...
          <div class="col-md-3 col-sm-3 col-xs-3">
            ...
            <br>
            <address>
              <b>Order #{{ $order->order_code }}</b><br>
              <b>Date created:</b> {{ date_format($order->created_at, 'd/m/Y') }}<br>
              <b>Pay:</b> {{ $order->payment_method->name }}
            </address>
          </div>
        </div>
      </div>
            ...
            <tbody>
              ...
              @foreach($order->order_details as $key => $order_detail)
                ...
                <tr>
                  ...
                  <td><span style="color: #f30;">{{ number_format($order_detail->price,0,',','.') }} $</span></td>
                  <td><span style="color: #f30;">{{ number_format($order_detail->price * $order_detail->quantity,0,',','.') }} $</span></td>
                </tr>
              @endforeach
              <tr>
                  <td colspan="7" style="text-align: right;"><b>Total = <span style="color: #f30;">{{ number_format($price,0,',','.') }} $</span></b></td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div class="row">
        ...
        <div class="col-xs-6">
          ...
          <div class="table-responsive">
            <table class="table">
              <tr>
                <th style="width:50%">Total Money:</th>
                <td><span style="color: #f30;">{{ number_format($price,0,',','.') }} $</span></td>
              </tr>
              <tr>
                <th>Đã Thanh Toán:</th>
                @if(Str::contains($order->payment_method->name, 'Online Payment'))
                <td><span style="color: #f30;">{{ number_format($price,0,',','.') }} $</span></td>
                @else
                <td><span style="color: #f30;">0 $</span></td>
                @endif
              </tr>
              <tr>
    ...
    <div class="row no-print">
      <div class="col-xs-12">
        <button class="btn btn-success btn-print pull-right"><i class="fa fa-print"></i> Print Invoice</button>
      </div>
    </div>
  </section>
  <!-- /.content -->
